...
@Composable fun MutexChipGroup(chips: List<Pair<String, Int>>, selectedIdState: MutableState<Int?>,
    ...
    Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = arrangement) {
        chips.forEach { chip ->
            val id = chip.second
            // Mutually-exclusive, non-cancelable single-select: tapping a chip selects it; tapping the
            // already-selected chip does nothing (you can't deselect into an ambiguous "no mode" state).
            ToggleableChip(chip.first, id == selectedId) { if (it) selectedIdState.value = id }}}
}
