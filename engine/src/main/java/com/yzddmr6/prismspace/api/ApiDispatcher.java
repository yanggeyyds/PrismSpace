...
import com.yzddmr6.prismspace.util.Permissions;
...
class ApiDispatcher {
	...
	static String verifyCaller(final Context context, final Intent intent, @Nullable String pkg, int uid) {
		...
		// Legacy verification is not supported inside PrismSpace without INTERACT_ACROSS_USERS on Android P+, due to MATCH_ANY_USER being restricted.
	...
	static String dispatch(final Context context, final Intent intent) {
		...
		case Api.latest.ACTION_UNFREEZE:
			...
			return processPackageUri(intent, null, pkg -> PrismManager.ensureAppHiddenState(context, pkg, hidden));
		case Api.latest.ACTION_LAUNCH:
	...
	private static String launchActivity(final Context context, final Intent intent) {
		...
		if ("package".equals(scheme)) {
			...
			final String free_to_launch = PrismManager.ensureAppFreeToLaunch(context, pkg);
			if (! free_to_launch.isEmpty()) return free_to_launch;
			return PrismManager.launchApp(context, pkg, Process.myUserHandle()) instanceof LaunchResult.Ok ? null : "no_launcher_activity";
		}
	...
	private static String processPackageUri(final Intent intent, final Function<Stream<String>, String> batch_dealer, final Predicate<String> dealer) {
		...
		try {
			...
			return "Internal exception: " + e;		// PrismSpace might be have been deactivated or not set up yet.
		}
	}

	private static final Map<String/* pkg */, Integer/* signature hash */> sVerifiedCallers = new ArrayMap<>(0);
