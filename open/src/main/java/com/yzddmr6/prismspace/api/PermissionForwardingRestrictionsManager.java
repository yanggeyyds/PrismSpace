...
import com.yzddmr6.prismspace.DelegatedScopeAuthorization;
import com.yzddmr6.prismspace.RestrictedBinderProxy;
import com.yzddmr6.prismspace.util.PackageManagerCompat;
...
/**
 * Allow client to call {@link RestrictionsManager#requestPermission(String, String, PersistableBundle)} in owner user when PrismSpace is not device owner.
 *
...
public class PermissionForwardingRestrictionsManager extends DerivedRestrictionsManager {
	...
	private boolean ensureCallerMatchesPackage(final String pkg) {
		try {
			final int uid = new PackageManagerCompat(mContext).getPackageUid(pkg);
	...
	@SuppressWarnings("SpellCheckingInspection") private static final String TAG = "Prism.PFRM";
}
