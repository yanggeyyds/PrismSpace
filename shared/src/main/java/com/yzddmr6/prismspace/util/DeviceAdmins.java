...
public class DeviceAdmins {
	...
	private static ComponentName queryComponentName(final Context context) {
		...
		if (! admins.isEmpty()) {
			final ResolveInfo admin = admins.stream()
					.filter(resolve -> "com.yzddmr6.prismspace.PrismDeviceAdminReceiver".equals(resolve.activityInfo.name))
					.findFirst().orElse(admins.get(0));
			sDeviceAdminComponent = new ComponentName(Modules.MODULE_ENGINE, admin.activityInfo.name);
		...
		return new ComponentName(Modules.MODULE_ENGINE, "com.yzddmr6.prismspace.PrismDeviceAdminReceiver");	// Fallback
	}
