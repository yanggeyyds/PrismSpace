...
class AppInfoForwarderActivity : CallerAwareActivity() {
	...
	private fun buildTargetIntent(pkg: String, user: UserHandle?, target: Intent): Intent {
		...
		if (detailsActivity != null) {
			...
			if (isCallerNotSettings && ! isCallerPrismButNotForwarder(caller)) {    // Started by 3rd-party app or this forwarder itself
			...
			if (isCallerNotSettings && user != null && user != Users.current()) {
				if (ActivityShuttle.canForwardTo(this, user)) details.component = ActivityShuttle.getForwarder(this) // Forwarding added in PrismProvisioning
		...
		if (isCallerNotSettings && detailsActivity != null) {
			if (user != null && ! user.isParentProfile() && Users.isProfileManagedByPrism(this, user)) {    // Use mainland resolve to replace the misleading forwarding-resolved "Switch to work profile".
		...
		if (isCallerNotSettings && isCallerPrismButNotForwarder(caller))
	...
	private fun isCallerPrismButNotForwarder(caller: String?): Boolean {
...
private const val TAG = "Prism.AIF"
