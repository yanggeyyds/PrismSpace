...
/**
 * App watcher for unfrozen apps in PrismSpace, for convenient refreezing.
 *
 * Created by Oasis on 2019-2-27.
 */
@RequiresApi(O) class PrismAppWatcher : BroadcastReceiver() {
	...
	override fun onReceive(context: Context, intent: Intent) {
		...
		when (action) {
			...
			ACTION_DISMISS  -> NotificationIds.PrismAppWatcher.cancel(context, if ("package" == data.scheme) ssp else data.toString())
			ACTION_PACKAGE_REMOVED,
			Intent.ACTION_PACKAGE_FULLY_REMOVED -> NotificationIds.PrismAppWatcher.cancel(context, ssp)
			DevicePolicies.ACTION_PACKAGE_UNFROZEN ->
				try {
					if (NotificationIds.PrismAppWatcher.isBlocked(context)) return
					...
					NotificationIds.PrismAppWatcher.cancel(context, ssp) }
			ACTION_REVOKE_PERMISSION -> {
				...
				NotificationIds.PrismAppWatcher.cancel(context, data.toString()) }}
	}
	...
	private fun refreeze(context: Context, pkg: String, watching_permissions: List<String>?) {
		...
		for (granted_permission in grantedPermissions) try {
			...
			NotificationIds.PrismAppWatcher.post(context, tag) {
				buildShared(context, pkg, com.yzddmr6.prismspace.shared.R.color.accent)
	...
	/** This provider tracks all freezing and unfreezing events triggered by other modules within the default process of PrismSpace  */
	class AppStateTracker : PseudoContentProvider() {

		override fun onCreate() = false.also { Handler(Looper.getMainLooper()).post {
			ContextCompat.registerReceiver(context(), PrismAppWatcher(),
	...
	companion object {
		...
		private fun startWatching(context: Context, info: PackageInfo) {
			...
			NotificationIds.PrismAppWatcher.post(context, pkg) {
				buildShared(context, pkg, com.yzddmr6.prismspace.shared.R.color.primary)
		...
		private fun makePendingIntent(context: Context, action: String, scheme: String, ssp: String, extras: ((Intent).() -> Unit)? = null)
				= PendingIntent.getBroadcast(context, 0, Intent(action).setClass(context, PrismAppWatcher::class.java).apply {
...
private const val TAG = "Prism.AppWatcher"
